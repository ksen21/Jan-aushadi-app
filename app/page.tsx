...
function SkeletonLine({ w = "full", h = "4" }: { w?: string; h?: string }) {
  return (
    <div
      className={`h-${h} w-${w} animate-pulse rounded-md bg-slate-200`}
    />
  );
}

function KendraSkeletonCard() {
  return (
    <div className="rounded-md border border-slate-200 p-4 space-y-3">
      <SkeletonLine w="2/3" h="5" />
      <SkeletonLine w="full" h="4" />
      <SkeletonLine w="1/4" h="4" />
      <div className="h-10 w-40 animate-pulse rounded-md bg-slate-200" />
    </div>
  );
}

function SearchSkeletonResult() {
  return (
    <section className="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm space-y-4">
      <SkeletonLine w="1/3" h="4" />
      <SkeletonLine w="2/3" h="8" />
      <SkeletonLine w="1/2" h="4" />
      <SkeletonLine w="1/4" h="4" />
      <div className="h-11 w-full animate-pulse rounded-md bg-emerald-100" />
...
export default function Home() {
  ...
  async function handleImageChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) { setImageBase64(null); setImageName(""); setMimeType(null); return; }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.detail || "Could not find a match.");
      setResult(data);
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "Could not find a match. Try typing the name manually.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  function resetSearch() {
    setMedicineName(""); setImageBase64(null); setImageName("");
    setMimeType(null); setResult(null); setError(""); setView("search");
  }
  ...
  async function fetchKendras(lat: number, lng: number) {
    ...
    setKendraError("");
    try {
      const response = await fetch(`${API_BASE_URL}/api/kendra-search?lat=${lat}&lng=${lng}`);
      const data = await response.json();
      if (!response.ok) throw new Error(data.detail || "Could not find nearby kendras.");
      setKendras(data);
    } catch (requestError) {
      setKendraError(requestError instanceof Error ? requestError.message : "Could not find nearby kendras.");
    } finally {
  ...
  function handleFindKendras() {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => fetchKendras(position.coords.latitude, position.coords.longitude),
  ...
  function handleManualLocalitySubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!coords) { setKendraError("Please enter an area or locality name."); return; }
  ...
  return (
    <main className="min-h-screen bg-[#f7faf8] text-slate-950">
      <section className="mx-auto flex min-h-screen w-full max-w-3xl flex-col px-5 py-8 sm:px-8">
        <div className="mb-7">
          <p className="text-sm font-medium text-emerald-700">Indore generic medicine lookup</p>
          <h1 className="mt-2 text-3xl font-semibold tracking-normal text-slate-950 sm:text-4xl">
            Jan Aushadi Finder
          </h1>
          <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">
            Type a medicine name or upload a clear photo of the strip or box to find a likely generic name match.
          </p>
        </div>
        ...
        {view === "search" ? (
          <>
            ...
            >
              <label htmlFor="medicine-name" className="text-sm font-semibold text-slate-900">
                Medicine name
              </label>
              <input
                id="medicine-name"
                value={medicineName}
                onChange={(event) => { setMedicineName(event.target.value); setResult(null); setError(""); }}
            ...
            {isLoading ? <SearchSkeletonResult /> : null}
              ...
              <section className="mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                {result.matchedName && result.genericName ? (
                  <>
                    ...
                    <h2 className="mt-2 text-2xl font-semibold text-slate-950">{result.genericName}</h2>
                    <p className="mt-2 text-sm text-slate-600">Matched from: {result.matchedName}</p>
                    {result.strengths?.length ? (
                      <p className="mt-1 text-sm text-slate-600">
                        Common strengths: {result.strengths.join(", ")}
                      </p>
                    ) : null}
                    ...
                    {result.aiGenerated ? (
                      <div className="mt-3 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-950">
                        This match was identified by AI and is not yet verified against the official Jan Aushadi list.
                  ...
                  <>
                    <h2 className="text-xl font-semibold text-slate-950">No confident match found</h2>
                    <p className="mt-2 text-sm leading-6 text-slate-600">
                      {result.message || "Try typing the medicine name manually using the brand name printed on the strip or box."}
                    </p>
                  </>
                )}
              </section>
            ...
            <div className="mt-5 rounded-md bg-amber-50 p-4 text-sm leading-6 text-amber-950">
              This app only matches medicine names to likely generic names. It does not provide dosage, treatment,
              prescription, or substitution advice. Confirm with a doctor or pharmacist before changing any medicine.
            </div>
          </>
        ) : (
          /* ── Kendra view ─────────────────────────────────────── */
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <h2 className="mt-3 text-xl font-semibold text-slate-950">Nearby Jan Aushadi Kendra</h2>

            {/* ── Loading state with "Finding..." text + skeletons ── */}
            {isLoadingKendras ? (
              <div className="mt-4 space-y-4">
                <div className="flex items-center gap-2 text-sm font-medium text-emerald-700">
                  {/* Animated spinner dot */}
                  <span className="inline-block h-3 w-3 animate-ping rounded-full bg-emerald-500 opacity-75" />
            ...
            {kendraError ? (
              <div className="mt-4 rounded-md border border-red-200 bg-red-50 p-4 text-sm text-red-800">{kendraError}</div>
            ) : null}
            ...
            {locationDenied && !isLoadingKendras ? (
              ...
              >
                <label htmlFor="manual-locality" className="text-sm font-semibold text-amber-950">
              ...
              <ul className="mt-5 space-y-3">
                {kendras.map((kendra) => {
                  ...
                  return (
                    ...
                    >
                      <p className="font-semibold text-slate-950">{kendra.name}</p>
                      <p className="mt-1 text-sm text-slate-600">{kendra.address}</p>
                      <p className="mt-1 text-sm font-medium text-emerald-700">{kendra.distanceKm} km away</p>
                      {whatsappLink ? (
                        <a
                          href={whatsappLink}
                          target="_blank"
                        ...
                        <p className="mt-3 text-sm text-slate-400">WhatsApp number not available for this kendra.</p>
                      )}
                    </li>
                  );
                })}
